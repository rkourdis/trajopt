    ...
    task_constraints = [
        (
            ...
            lambda kv, **kwargs: [
                ...
                Constraint(kv.q[3:] - create_state_vector(kwargs["robot"].robot, SOLO_FOLDED_JOINT_MAP)[3:]),
        ...
        (
            ...
            lambda kv, **kwargs: [
                ...
                Constraint(kv.q[3:] - load_robot_pose(Pose.SOLO_STANDING_V)[0][3:]),
                Bound(kv.q[:2]),
                Bound(kv.v)
            ]
        ),
...
JumpTaskInPlace: Task = Task(
    ...
    task_constraints = [
        (
            ...
            lambda kv, **kwargs: [
                Constraint(kv.q - load_robot_pose(Pose.SOLO_STANDING_V)[0]),
                Bound(kv.v)
            ]
        ),
        (
        ...
        (
            ...
            lambda kv, **kwargs: [
                Bound(kv.q[2], lb = kwargs["robot"].floor_z + 0.06, ub = ca.inf),
            ]
        )
    ],
)
...
# NOTE: I've broken my Solo doing this and JumpTaskBwd. I don't remember exactly
...
JumpTaskFwd: Task = Task(
    ...
    task_constraints = [
        (
            ...
            lambda kv, **kwargs: [
                Constraint(kv.q[:2] - load_robot_pose(Pose.SOLO_STANDING_V)[0][:2]),
                Constraint(kv.q[3:] - load_robot_pose(Pose.SOLO_STANDING_V)[0][3:]),
                Bound(kv.v)
            ]
        ),
        (
            ...
            lambda kv, **kwargs: [
                ...
                Bound(kv.q[2], kwargs["robot"].floor_z + 0.2, ca.inf),
                Bound(kv.q[3:6]),
        ...
        (
            ...
            lambda kv, **kwargs: [
                Bound(kv.q[2], lb = kwargs["robot"].floor_z + 0.08, ub = ca.inf),
                Constraint(kwargs["fk"](kv.q)["knees"][:, 2], lb = kwargs["robot"].floor_z + 0.02, ub = ca.inf),
                Bound(kv.v[3]),
                Bound(kv.v[5]),
            ]
        )
...
LR_Symmetry_Constraints = lambda kv, **kwargs: [
    ...
    Constraint(kv.q[kwargs["robot"].q_off("HR_KFE")] - kv.q[kwargs["robot"].q_off("HL_KFE")]),
    Constraint(kv.q[kwargs["robot"].q_off("HR_HFE")] - kv.q[kwargs["robot"].q_off("HL_HFE")]),
    Constraint(kv.q[kwargs["robot"].q_off("HR_HAA")] + kv.q[kwargs["robot"].q_off("HL_HAA")]),
]
...
HFE_Limit_Constraints = lambda kv, **kwargs: [
    Bound(kv.q[kwargs["robot"].q_off("FL_HFE")], lb = -5 * np.pi / 4, ub = np.pi/2),
    Bound(kv.q[kwargs["robot"].q_off("FR_HFE")], lb = -5 * np.pi / 4, ub = np.pi/2),
    Bound(kv.q[kwargs["robot"].q_off("HL_HFE")], lb = -np.pi/2,       ub = 5 * np.pi / 4),
    Bound(kv.q[kwargs["robot"].q_off("HR_HFE")], lb = -np.pi/2,       ub = 5 * np.pi / 4),
]
    ...
    task_constraints = [
        ...
        (
            ...
            lambda kv, **kwargs: [
                ...
                Bound(kv.q[2], lb = kwargs["robot"].floor_z + 0.12, ub = ca.inf),

                # Don't allow the hind knees to go below +2cm:
                Constraint(
                    kwargs["fk"](kv.q)["knees"][2:, 2],
                    lb = kwargs["robot"].floor_z + 0.02,
                    ub = ca.inf
                )
            ] +
    ...
    task_constraints = [
        (
            ...
            lambda kv, **kwargs: [
                ...
                Bound(kv.q[2], lb = kwargs["robot"].floor_z + 0.2, ub = ca.inf),
            ]
        ),
        (
